...
const VALID_WEIGHTS: readonly QuestionWeight[] = [1, -1];

function validateQuestion(q: QuestionnaireQuestion, index: number): void {
  if (!q.text?.trim()) {
...
function validateQuestions(questions: QuestionnaireQuestion[]): void {
...
export async function updateQuestionnaire(
  ...
  const keys = Object.keys(updates).filter(
    (k) => updates[k as keyof QuestionnaireUpdate] !== undefined,
  );
  if (keys.length === 0) {
...
export async function reorderQuestions(
  ...
  const reordered: QuestionnaireQuestion[] = questionIds.map((qid, index) => ({
