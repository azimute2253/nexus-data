...
export function calculateScore(answers, questions) {
    ...
    for (const answer of answers) {
        ...
        if (weight === undefined) {
            throw new Error(`Pergunta "${answer.question_id}" não encontrada no questionário`);
        }
...
export function validateAnswers(answers, questions) {
    ...
    for (const a of answers) {
        if (!questionIds.has(a.question_id)) {
            throw new Error(`Resposta referencia pergunta inexistente: "${a.question_id}"`);
        }
    }
 ...
 */
export async function getAssetScore(assetId, questionnaireId) {
 ...
 */
export async function saveAssetScore(assetId, questionnaireId, answers, questionnaire, userId, walletId) {
 ...
 */
export async function deleteAssetScore(assetId, questionnaireId) {
 ...
 */
export async function getScoresByQuestionnaire(walletId, questionnaireId) {
    const { data, error } = await getAnonClient()
        .from(TABLE)
        .select('*')
