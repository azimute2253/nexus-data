 ...
 */
export function getDeviationLevel(deviationPct) {
    const abs = Math.abs(deviationPct);
    if (abs <= ALIGNED_THRESHOLD_PP)
        return 'aligned';
    if (deviationPct > SIGNIFICANT_THRESHOLD_PP)
        return 'significant-over';
    if (deviationPct < -SIGNIFICANT_THRESHOLD_PP)
        return 'significant-under';
    if (deviationPct > 0)
        return 'minor-over';
    return 'minor-under';
}
 ...
 */
export function getBarWidthPct(deviationPct) {
    const abs = Math.abs(deviationPct);
    if (abs <= ALIGNED_THRESHOLD_PP)
        return 0;
...
export function DeviationBar({ deviationPct, targetMissing = false }) {
    if (targetMissing) {
        return (_jsx("span", { className: "text-sm text-gray-400", "aria-label": "Sem meta definida", children: "\u2014" }));
    }
    ...
    return (_jsxs("div", { className: "flex items-center gap-2", title: `${sign}${deviationPct.toFixed(2)}pp ${label}`, role: "meter", "aria-valuenow": Math.abs(deviationPct), "aria-valuemin": 0, "aria-valuemax": MAX_DEVIATION_PP, "aria-label": `Desvio: ${sign}${deviationPct.toFixed(2)}pp`, children: [_jsx("div", { className: "h-2 flex-1 rounded-full bg-gray-100", "aria-hidden": "true", children: _jsx("div", { className: `h-full rounded-full transition-all ${styles.bar}`, style: { width: `${widthPct}%` } }) }), _jsxs("span", { className: `shrink-0 text-xs font-medium ${styles.text}`, children: [sign, deviationPct.toFixed(1), "pp"] })] }));
}
